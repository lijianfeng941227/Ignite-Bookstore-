...
public class DBGeneral {
    ...
    public static String[] getName(int c_id) {
        ...
        try {
            ...
            System.out
                .println("Executing SQL: SELECT c_fname,c_lname FROM customer WHERE c_id = "
                        + c_id);
            ex.printStackTrace();
        } finally {
            closeResultSet(rs);
    ...
    public static Book getBook(int i_id) {
        ...
        try {
            ...
            if (!rs.next()) {
                rs.close();
                throw new SQLException("Execute From Hazelcast rs is empty");
            }
            book = new Book(rs);
            // con.commit();
        } catch (java.lang.Exception ex) {
            System.out
                .println("================================================================================");
            System.out
                .println("Executing SQL: SELECT * FROM item,author WHERE item.i_a_id = author.a_id AND i_id = "
                        + i_id);
            try {
                statement.close();
                statement = con
                    .prepareStatement("SELECT * FROM item,author WHERE item.i_a_id = author.a_id AND i_id = ? limit 1");
                statement.setInt(1, i_id);
                rs = statement.executeQuery();
                // Results
                if (!rs.next()) {
                    throw new SQLException(
                            "Execute From DataBase : rs is empty");
                } else {
                    System.out.println("Exist in DataBase");
                }
                rs.close();
                statement = con
                    .prepareStatement("SELECT * FROM item WHERE i_id ="
                            + i_id);
                rs = statement.executeQuery();
                int i_a_id = -1;
                ...
                while (rs.next()) {
                    ...
                    i_a_id = rs.getInt("i_a_id");
                    break;
                }
                rs.close();
                statement.close();
                statement = con
                    .prepareStatement("SELECT * FROM author WHERE a_id ="
                            + i_a_id);
                rs = statement.executeQuery();
                rsmd = rs.getMetaData();
    ...
    // added by wangyanshi for customer consistency
    // c_id is the key of customer from session
    public static Customer getCustomer(Integer c_id) {
        ...
        try {
            ...
            con = getConnection();
            statement = con
                .prepareStatement("SELECT * FROM customer, address, country WHERE customer.c_addr_id = address.addr_id AND address.addr_co_id = country.co_id AND customer.c_id = ?");
    ...
    public static int createEmptyCart() {
        ...
        try {
            ...
            // System.out.println("createEmptyCart() : " + SHOPPING_ID);
            // con.commit();
        } catch (java.lang.Exception ex) {
            ex.printStackTrace();
        } finally {
    ...
    public static Cart doCart(int SHOPPING_ID, Integer I_ID, Vector ids,
        ...
        try {
            ...
            cart = DBGeneral.getCart(con, SHOPPING_ID, 0.0);
    ...
    private static void addItem(Connection con, int SHOPPING_ID, int I_ID) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // added by wangyanshi
                // System.out.println("addItem() : " + SHOPPING_ID);
                //
                ...
                // added by wangyanshi to back up the shopping_cart_line primary
                // keys for delete
                HashSet<Integer> tmpHashSet = ShoppingCartLineRecord.shoppingcartlineHashmap
                    .get(SHOPPING_ID);
                if (tmpHashSet == null) {
                    tmpHashSet = new HashSet<Integer>();
                    tmpHashSet.add(I_ID);
                    ShoppingCartLineRecord.shoppingcartlineHashmap.put(
                            SHOPPING_ID, tmpHashSet);
                } else {
                    tmpHashSet.add(I_ID);
                }
                // adding ended
        ...
        try {
            for (i = 0; i < ids.size(); i++) {
                ...
                if (QTY == 0) { // We need to remove the item from the cart
                    ...
                    // added by wangyanshi to back up the shopping_cart_line
                    // primary keys for delete
                    HashSet<Integer> tmpHashSet = ShoppingCartLineRecord.shoppingcartlineHashmap
                        .get(SHOPPING_ID);
                    if (tmpHashSet != null) {
                        ...
                        // SHOPPING_ID);
                        tmpHashSet.remove(I_ID);
                        if (tmpHashSet.size() == 0) {
                            ShoppingCartLineRecord.shoppingcartlineHashmap
                                .remove(SHOPPING_ID);
                        }
                    } else {
                        System.out
                            .println("refreshCart(): ShoppingCartLineRecord.shoppingcartlineHashmap.get("
                                    + SHOPPING_ID + ") is null");
                    }
                    // adding ended
    ...
    private static void addRandomItemToCartIfNecessary(Connection con,
        ...
        try {
            ...
            // get_cart =
            // con.prepareStatement("SELECT COUNT(*) from shopping_cart_line where scl_sc_id = ?");
            ...
            // }
            int num = 0;
            // added by yanshi.wys to replace count(*) 2013/01/30
            if(hzinstance == null) {
            	hzinstance = Hazelcast.getHazelcastInstanceByName("IMDG");
            }
            IMap<String, Object> map = hzinstance.getMap("shopping_cart_line");
            Set<String> keysSet = map.keySet(new SqlPredicate("$cglib_prop_scl_sc_id=" + SHOPPING_ID));
            num = keysSet.size();
            // added over
            if (num == 0) {
                // Cart is empty
 ...
 //           cust.c_expiration = new Date(System.currentTimeMillis() + 7200000);// milliseconds
    ...
    public static void clearCart(Connection con, int shopping_id) {
        ...
        try {
            ...
            // changed by wangyanshi to replace delete based value with
            // ShoppingCartLineRecord
            // old version ...
            // statement =
            // con.prepareStatement("DELETE FROM shopping_cart_line WHERE scl_sc_id = ?");
            // Set parameter
            // statement.setInt(1, shopping_id);
            // statement.executeUpdate();
            // by dch
            ...
            if (tmpHashSet != null) {
                ...
                // " : " + shopping_id);
                for (Integer integer : tmpHashSet) {
                    statement.setInt(1, shopping_id);
                    statement.setInt(2, integer.intValue());
                    statement.executeUpdate();
                }
                ShoppingCartLineRecord.shoppingcartlineHashmap
                    .remove(shopping_id);
    ...
    public static int enterAddress(
        ...
        try {
            ...
            /*
             * Add by wangyanshi to debug
             */
            if (!rs.next()) {
                System.out.println("SELECT co_id FROM country WHERE co_name = "
                        + country);
                ResultSet rs1 = get_co_id
                    .executeQuery("SELECT co_id FROM country");
